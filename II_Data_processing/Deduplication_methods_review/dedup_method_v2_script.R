###############################################################################
#                                                                             #
#    PETER G DEDUPLICATION METHODOLOGY VERSION 2 (V2)                         #
#    Duplicate removal by selection of data source with greatest number        #
#    of records WITH COORDS per instcode (1 data source per instcode)         #
#                                                                             #
#    Deduplication Method:                                                    #
#    - For each organization (inst_code), only ONE data source is kept:       #
#      whichever source has the greatest number of records WITH coordinates   #
#      for that inst_code.                                                    #
...
#                                                                             #
#      DEDUPLICATION STEPS:                                                   #
#         - Step 1: For each inst_code, keep only the source with most        #
#                   records WITH coords                                       #
#           (prioritze Genesys if same number of records with coords)         #
#         - Step 2: Remove duplicates by DOI, prioritizing records with coords#
#                   (then Genesys)                                            #
#         - Step 3: Remove duplicates by composite ID (ACCENUMB+inst_code+    #
#                   genus_species_WFO), prioritizing coords (then Genesys)    #
#         - Step 4: After merging back in non-Genesys/WIEWS data, repeat      #
#                   "most coords per inst_code" selection                     #
...
# STEP 1: Genesys or WIEWS by COORDINATE record count (prioritize Genesys if same number of records with coords)
genesys_df$has_coords <- !is.na(genesys_df$latitude) & genesys_df$latitude != "" & !is.na(genesys_df$longitude) & genesys_df$longitude != ""
wiews_df$has_coords <- !is.na(wiews_df$latitude) & wiews_df$latitude != "" & !is.na(wiews_df$longitude) & wiews_df$longitude != ""

genesys_coord_counts <- genesys_df %>%
  group_by(inst_code) %>%
  summarize(n_genesys = sum(has_coords, na.rm=TRUE))
wiews_coord_counts <- wiews_df %>%
  group_by(inst_code) %>%
  summarize(n_wiews = sum(has_coords, na.rm=TRUE))

instcode_counts <- full_join(genesys_coord_counts, wiews_coord_counts, by = "inst_code") %>%
  mutate(
    ...
    keep_source = case_when(
      ...
      n_genesys == n_wiews ~ "Genesys" # tie-breaker
    )
  )
...
# STEP 2: Remove duplicate DOI, prioritize with coords then Genesys
gen_wiews_df_step1 <- gen_wiews_df
gen_wiews_df <- gen_wiews_df %>%
  mutate(has_coords = !is.na(latitude) & latitude != "" & !is.na(longitude) & longitude != "") %>%
  arrange(DOI, desc(has_coords), desc(data_source == "Genesys")) %>%
  group_by(DOI) %>%
  mutate(rowno = row_number()) %>%
  filter(is.na(DOI) | DOI == "" | rowno == 1) %>%
  ungroup() %>%
  select(-rowno, -has_coords)
...
# STEP 3: Remove duplicate IDs, prioritize with coords then Genesys
gen_wiews_df_step2 <- gen_wiews_df

# -- Clean fields and create unique ID (uses genus_species_WFO as requested) --
...
gen_wiews_df <- gen_wiews_df %>%
  mutate(has_coords = !is.na(latitude) & latitude != "" & !is.na(longitude) & longitude != "") %>%
  arrange(ID, desc(has_coords), desc(data_source == "Genesys")) %>%
  group_by(ID) %>%
  mutate(rowno = row_number()) %>%
  filter(is.na(ID) | ID == "" | rowno == 1) %>%
  ungroup() %>%
  select(-rowno, -ID, -has_coords)

dup_annotations[[length(dup_annotations)+1]] <- dup_annot(gen_wiews_df_step2, gen_wiews_df, "step3_id_dedup")
species_summary_list[['step3']] <- per_source_summary(
  bind_rows(gen_wiews_df, bgci_df, cano_df, gbif_living_df), "step3")

# STEP 4: Most records WITH coords per inst_code after merging in BGCI, Cano, GBIF
genesys_df <- gen_wiews_df %>% filter(data_source == "Genesys")
wiews_df   <- gen_wiews_df %>% filter(data_source == "WIEWS")
genesys_df$has_coords      <- !is.na(genesys_df$latitude) & genesys_df$latitude != "" & !is.na(genesys_df$longitude) & genesys_df$longitude != ""
wiews_df$has_coords        <- !is.na(wiews_df$latitude) & wiews_df$latitude != "" & !is.na(wiews_df$longitude) & wiews_df$longitude != ""
bgci_df$has_coords         <- FALSE
cano_df$has_coords         <- FALSE
gbif_living_df$has_coords  <- !is.na(gbif_living_df$latitude) & gbif_living_df$latitude != "" & !is.na(gbif_living_df$longitude) & gbif_living_df$longitude != ""
...
instcode_src_counts <- combined_df_step3 %>%
  ...
  summarize(n = sum(has_coords, na.rm=TRUE), .groups = "drop")
...
# STEP 5: Organization assignment and select genebanks, botanic gardens
org_guide <- org_guide %>%
...
write_xlsx(
  ...
  file.path(output_dir, "deduplication_summary_v2.xlsx")
)
